    ...
    for out_dir_str, label in zip(args.out_dir, args.label):
        ...
        if equity_history_path.exists():
            with open(equity_history_path, "r") as f:
                ...
                for row in reader:
                    dt = _parse_timestamp(row.get("date") or "")
                    if not dt:
                        try:
...
def main():
    ...
    print(f"DEBUG: Generated {len(all_equities)} equity curves.")
    if not all_equities:
        print("DEBUG: No data found to plot!")
        return
    ...
    print(f"DEBUG: Writing graph to {os.path.abspath(args.out)}")
