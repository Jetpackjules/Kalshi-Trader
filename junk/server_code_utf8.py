...
class LiveTraderV4:
        ...
        for attempt in range(3):
            ...
            try:
                ...
                if resp.status_code == 429:
                    print(f"ΓÜá∩╕Å API Rate Limit (429) - Attempt {attempt+1}. Backing off...", flush=True)
                    time.sleep(1.0 + 0.2 * attempt)
                    continue

                # Retry on transient 5xx
                if resp.status_code in RETRYABLE_HTTP:
                    print(f"ΓÜá∩╕Å API Server Error ({resp.status_code}) - Attempt {attempt+1}. Backing off...", flush=True)
                ...
                print(f"ΓÜá∩╕Å API Network Error ({method} {path}) - Attempt {attempt+1}: {repr(e)}", flush=True)
                time.sleep(0.5 * (2 ** attempt) + 0.1 * attempt)
                last_err = e
        
        print(f"Γ¥î API Request FAILED after 3 attempts ({method} {path}): {repr(last_err)}", flush=True)
        return None
    ...
    def place_real_order(self, ticker, qty, price, side, expiry_ts):
        ...
        if resp is None:
            print(f"Γ¥î FAIL: No response placing order (ticker={ticker} side={side} qty={qty} price={price})", flush=True)
            return False, 0.0

        if resp.status_code == 201:
            print(f"Γ£à ORDER: {ticker} {side.upper()} {qty} @ {price} (Exp: {expiry_ts.strftime('%H:%M:%S')})", flush=True)
            return True, calculate_convex_fee(price, qty)

        print(f"Γ¥î FAIL {resp.status_code}: {resp.text} payload={payload}", flush=True)
        return False, 0.0
